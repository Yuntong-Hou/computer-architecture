# Paper Metadata

- Title: AVATAR: A Variable-Retention-Time (VRT) Aware Refresh for DRAM Systems
- Chinese Title: AVATAR：面向 DRAM 系统的 Variable-Retention-Time 感知刷新
- Authors: Moinuddin K. Qureshi, Dae-Hyun Kim, Samira Khan, Prashant J. Nair, Onur Mutlu
- Year: 2015
- Venue / Journal / Conference: DSN 2015
- DOI: 未找到
- arXiv ID: 未找到
- URL: https://github.com/Yuntong-Hou/computer-architecture/blob/main/LEC3/avatar-dram-refresh_dsn15.pdf
- PDF Source: paper reading/sources/LEC3/avatar-dram-refresh_dsn15.pdf
- Code / Project Page: 未找到
- Dataset: 24 DRAM chips; simulated 8Gb-64Gb DRAM systems
- Main Topic: DRAM refresh; variable retention time; ECC; reliability
- Keywords: AVATAR, VRT, DRAM refresh, ECC, scrubbing, multirate refresh, active VRT pool
- Reading Status: 第七批增强完成；full_translation 已按新标准扩写
- Full Text Available: Yes
- Notes: 说明：本文件基于已下载 PDF 的抽取文本生成中文学习材料；2026-05-12 已按“更完整翻译 + 硬件工程师视角”标准增强 `full_translation.zh.md`。
