# Paper Metadata

- Title: A Case for Richer Cross-layer Abstractions: Bridging the Semantic Gap with Expressive Memory
- Chinese Title: 用 Expressive Memory 弥合语义鸿沟：更丰富跨层抽象的案例
- Authors: Nandita Vijaykumar, Abhilasha Jain, Diptesh Majumdar, Kevin Hsieh, Gennady Pekhimenko, Eiman Ebrahimi, Nastaran Hajinazar, Phillip B. Gibbons, Onur Mutlu
- Year: 2018
- Venue / Journal / Conference: ISCA 2018
- DOI: 未找到
- arXiv ID: 未找到
- URL: https://github.com/Yuntong-Hou/computer-architecture/blob/main/LEC3/X-MEM_Expressive-Memory-for-Rich-Cross-Layer-Abstractions_isca18.pdf
- PDF Source: paper reading/sources/LEC3/X-MEM_Expressive-Memory-for-Rich-Cross-Layer-Abstractions_isca18.pdf
- Code / Project Page: 未找到
- Dataset: Simulation workloads for cache tiling portability and DRAM page placement
- Main Topic: Cross-layer memory abstraction; program semantics; cache and DRAM optimization
- Keywords: XMem, Expressive Memory, Atom, semantic gap, cache management, page placement
- Reading Status: 第六轮深度阅读完成
- Full Text Available: Yes
- Notes: 说明：本文件基于已下载 PDF 的抽取文本生成中文学习材料；`full_translation.zh.md` 采用逐节中文详译/译述方式，不提供逐字复刻式全文翻译。
