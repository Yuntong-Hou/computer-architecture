# Paper Metadata

- Title: The Application Slowdown Model: Quantifying and Controlling the Impact of Inter-Application Interference at Shared Caches and Main Memory
- Chinese Title: Application Slowdown Model：量化并控制共享 cache 与主存处跨应用干扰的影响
- Authors: Lavanya Subramanian, Vivek Seshadri, Arnab Ghosh, Samira Khan, Onur Mutlu
- Year: 2015
- Venue / Journal / Conference: MICRO 2015
- DOI: 10.1145/2830772.2830803
- arXiv ID: 未找到
- URL: https://github.com/Yuntong-Hou/computer-architecture/blob/main/LEC3/application-slowdown-model_micro15.pdf
- PDF Source: paper reading/sources/LEC3/application-slowdown-model_micro15.pdf
- Code / Project Page: 未找到
- Dataset: 100 multiprogrammed workloads; SPEC CPU applications; database workloads
- Main Topic: Shared-resource interference; slowdown estimation; cache/memory QoS
- Keywords: ASM, slowdown, cache access rate, auxiliary tag store, memory bandwidth partitioning, QoS
- Reading Status: 第七批增强完成；full_translation 已按新标准扩写
- Full Text Available: Yes
- Notes: 说明：本文件基于已下载 PDF 的抽取文本生成中文学习材料；2026-05-12 已按“更完整翻译 + 硬件工程师视角”标准增强 `full_translation.zh.md`。
