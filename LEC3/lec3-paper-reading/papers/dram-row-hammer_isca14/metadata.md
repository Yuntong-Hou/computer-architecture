# Paper Metadata

- Title: Flipping Bits in Memory Without Accessing Them: An Experimental Study of DRAM Disturbance Errors
- Chinese Title: 无需访问即可翻转内存位：DRAM disturbance errors 的实验研究
- Authors: Yoongu Kim, Ross Daly, Jeremie Kim, Chris Fallin, Ji Hye Lee, Donghyuk Lee, Chris Wilkerson, Konrad Lai, Onur Mutlu
- Year: 2014
- Venue / Journal / Conference: ISCA 2014
- DOI: 未找到
- arXiv ID: 未找到
- URL: https://github.com/Yuntong-Hou/computer-architecture/blob/main/LEC3/dram-row-hammer_isca14.pdf
- PDF Source: paper reading/sources/LEC3/dram-row-hammer_isca14.pdf
- Code / Project Page: 未找到
- Dataset: 129 DDR3 DRAM modules; 972 DRAM chips; Intel/AMD real systems; FPGA test platform
- Main Topic: RowHammer; DRAM disturbance errors; memory security and reliability
- Keywords: RowHammer, disturbance error, DRAM, PARA, ECC, refresh, clflush
- Reading Status: 第八批增强完成；full_translation 已按新标准扩写
- Full Text Available: Yes
- Notes: 说明：本文件基于已下载 PDF 的抽取文本生成中文学习材料；2026-05-12 已按“更完整翻译 + 硬件工程师视角”标准增强 `full_translation.zh.md`。
